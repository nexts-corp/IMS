...
//  }
  ...
  debug.print(0,"w_ip(%d.%d.%d.%d)\n",w_ip[0],w_ip[1],w_ip[2],w_ip[3]);
  return w_ip;
}
...
//  }
  ...
  debug.print(0,"w_sn(%d.%d.%d.%d)\n",w_sn[0],w_sn[1],w_sn[2],w_sn[3]);
  return w_sn;
}
...
//  }
  ...
  debug.print(0,"w_gw(%d.%d.%d.%d)\n",w_gw[0],w_gw[1],w_gw[2],w_gw[3]);
  return w_gw;
}
...
//  }
  ...
  debug.print(0,"w_mac(%d:%d:%d:%d:%d:%d)\n",w_mac[0],w_mac[1],w_mac[2],w_mac[3],w_mac[4],w_mac[5]);
  return w_mac;
}
...
//  }
  ...
  debug.print(0,"w_dns_ip(%d.%d.%d.%d)\n",w_dns_ip[0],w_dns_ip[1],w_dns_ip[2],w_dns_ip[3]);
  return w_dns_ip;
}
...
//  }
  ...
  debug.print(0,"w_serv_ip(%d.%d.%d.%d)\n",w_serv_ip[0],w_serv_ip[1],w_serv_ip[2],w_serv_ip[3]);
  return w_serv_ip;
}
...
//  }
  ...
  debug.print(0,"w_serv_name(%s)\n",w_serv_name);
  return w_serv_name;
}
uint16_t WIFI_API::get_serv_port(){
  EEPROM.get( ARD_SERV_PORT , w_serv_port);
  debug.print(0,"w_serv_port(%d)\n",w_serv_port);
  return w_serv_port;
}
uint8_t WIFI_API::get_dns_use(){
  EEPROM.get( ARD_DNS_USE , w_dns_use);
  debug.print(0,"w_dns_use(%02X)\n",w_dns_use);
  return w_dns_use;
}
uint8_t WIFI_API::get_debug_use(){
  EEPROM.get( ARD_DEBUG_USE , w_debug_use);
  debug.print(0,"w_debug_use(%02X)\n",w_debug_use);
  return w_debug_use;
}
uint8_t WIFI_API::get_connected_serv(){
  debug.print(0,"w_connected_serv(%02X)\n",w_connected_serv);
  return w_connected_serv;
}


void WIFI_API::set_connected_serv(uint8_t data){
  w_connected_serv = data;
  debug.print(0,"w_connected_serv(%2X)\n",w_connected_serv);
}
...
void WIFI_API::set_dns_use(uint8_t data){
  w_dns_use = data;
  EEPROM.put( ARD_DNS_USE, w_dns_use );
  EEPROM.commit();
  debug.print(0,"w_dns_use(%02X)\n",w_dns_use);
}
void WIFI_API::set_debug_use(uint8_t data){
  w_debug_use = data;
  EEPROM.put( ARD_DEBUG_USE, w_debug_use );
  EEPROM.commit();
  debug.print(0,"w_debug_use(%02X)\n",w_debug_use);
}
...
void WIFI_API::send_status(uint8_t data){
  ...
  buff[idx_w++] = (uint8_t)W_CONNECTED_SERV;  //CMD
...
void WIFI_API::send_uart_data(uint8_t *data, uint16_t len){
  ...
  //pkt_chsum = checksum((char*)&buff[3],pkt_length);
  pkt_chsum = checksum2_buf((char*)&buff[3],1,(char*)data,len);
  ...
  do{
    if(value[idx_r] == 0x7e){
      ...
      if(checksum((char*)&value[offset+3], size_pkt) != value[offset+3+size_pkt]){        // checksum error detection                  
          ...
          offset++;
          total = offset;                                                               // roll back total analyse  
          send_status(STATUS_CHECKSUM_FAIL);        
      }
...
void WIFI_API::response_config(int8_t cmd , uint8_t *data, uint16_t len){
  ...
  if(cmd == W_GET_DHCP){
    ...
    ret = connected_check();
    uint8_t conn = get_connected_serv();
    debug.print(0,"CONNECTED with server: %02X\n",conn);
    ...
    buf[idx_w++] = pkt_length & 0xFF;
    buf[idx_w++] = (uint8_t)W_CONFIG_RESP;  //Fram Type
    buf[idx_w++] = (uint8_t)W_GET_CONNECTED_SERV;     //CMD
    buf[idx_w++] = (uint8_t)conn;     //Payload
  ...
  if(cmd == W_RESTART){
    ...
    uint8_t debug_use = data[2];
    debug.print(0,"Debug Use Set: %02X\n",debug_use);
    ...
    uint8_t dns_ip_use = data[2];
    debug.print(0,"DNS Use Set: %02X\n",dns_ip_use);
    set_dns_use(dns_ip_use);
    ...
    buf[idx_w++] = pkt_length & 0xFF;
    buf[idx_w++] = (uint8_t)W_CONFIG_RESP;  //Fram Type
    buf[idx_w++] = (uint8_t)W_DNS_IP_MODE;     //CMD
    buf[idx_w++] = (uint8_t)0x00;     //Set Success
    pkt_chsum = checksum((char*)&buf[3],pkt_length);
    buf[idx_w++] = pkt_chsum;
  ...
  if(cmd == W_SET_DHCP){
   ...
   // set_pwd(str_buf);
    connect_server();
    ret = connected_check();
    //ret = get_connected_serv();
    debug.print(0,(const char*)"CONNECT Server.\n");
    ...
    buf[idx_w++] = pkt_length & 0xFF;
    buf[idx_w++] = (uint8_t)W_CONFIG_RESP;  //Fram Type
    buf[idx_w++] = (uint8_t)W_SET_CONNECT_SERV;     //CMD
...
uint8_t  checksum2_buf(char *buf1, uint16_t len1,char *buf2, uint16_t len2){
  int i = 0;
  char sum = 0;                                          
  //print_payload(buf,len);
  for (i = 0; i < len1; i++) {
