...
#define STATUS_CHECKSUM_FAIL      0x74
...
#define ARD_DNS_USE     (uint16_t)156 //156 <=> 156 (1 byte)
#define ARD_DEBUG_USE   (uint16_t)157 //157 <=> 157 (1 byte)
...
class WIFI_API{
  ...
  uint8_t get_dns_use();
  uint8_t get_debug_use();

  uint8_t get_connected_serv();
  ...
  void set_dns_use(uint8_t data);
  void set_debug_use(uint8_t data);

  void set_connected_serv(uint8_t data);
