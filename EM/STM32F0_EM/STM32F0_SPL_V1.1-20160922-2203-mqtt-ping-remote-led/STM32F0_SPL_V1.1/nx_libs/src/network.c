...
//		}
	...
	print_debug(0,"(Set Context)AT+CGDCONT=1,\"IP\",\r\n");
	ret = gsm_write((uint8_t*)"AT+CGDCONT=1,\"IP\",\r\n",20);
	HAL_Delay(500);
	ret = gsm_read(data_gsm.value,&data_gsm.length);
	if(ret){
		if(data_gsm.length > 0){
...
//			}
	...
	print_debug(0,"(MODE)AT+CIPMODE=0\r\n");
...
//			}
	...
	print_debug(0,"(Start Connect)AT+CIPSTART=\"TCP\",\"mqtt.codeunbug.com\",\"1883\"\r\n");
	ret = gsm_write((uint8_t*)"AT+CIPSTART=\"TCP\",\"mqtt.codeunbug.com\",\"1883\"\r\n",47);
	HAL_Delay(2000);
	ret = gsm_read(data_gsm.value,&data_gsm.length);
	if(ret){
		if(data_gsm.length > 0){
...
void net_callback(uint8_t *topic_val, uint16_t topic_len, uint8_t *payload_val, uint16_t payload_len, uint16_t packet_id){
	...
	if(strncmp((const char*)topic_val,"m001/data/led",13)==0){
		//Rrmote Receive
